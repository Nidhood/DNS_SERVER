...
public class Test {
    public static void main(String[] args) throws IOException {
        ...
        InetAddress serverAddress =  InetAddress.getByName("10.43.100.27");
        ...
        try {
            ...
            System.out.println("Servidor DNS configurado correctamente...");
            while (true) {
                ...
                int portClient = requestPacket.getPort(); // Obtiene el puerto del cliente.
                ...
                // Mandamos la query al Foreing Resolver:
                answerData = buildForeingResolverQuery(message);
                if(answerData.length != 0){
                    // Recibimos de nuevo el paquete del foreing resolver:
                    ...
                    responseData = buildResponse(answerData);
                    if(responseData.length != 0){
                        System.out.println("Sended: \t\t" + responseData.length + " bytes\n");
                        responsePacket = new DatagramPacket(responseData, responseData.length, clientAddress, portClient);
    ...
    static byte[] buildForeingResolverQuery(byte[] answer) throws IOException{
        InetAddress foreingResolverAddress = InetAddress.getByName("10.2.1.10"); // Direccion de foreing resolver.
        byte[] foreingResolver = new byte[0]; // Buffer que va a ir cargando todos los campos del mensaje DNS.
        ...
        DataInputStream dataInputStream = new DataInputStream(new ByteArrayInputStream(answer));
        ...
        String RCODEAnswer = "0000";
        String flagsResponseAux = QRAnswer + opCodeAnswer + AAAnswer + TCAnswer + RDAnswer + RAAnswer + ZAnswer + RCODEAnswer;
        ...
        int anCount = 0;  // Número deseado de registros de respuesta
        ...
        if(ipAddress != null) {
            ...
            dataOutputStream.writeShort(1);
            foreingResolver = byteArrayOutputStream.toByteArray();
            System.out.println("Asked FR: \t\t" + foreingResolver.length + " bytes");
            DatagramSocket socket = new DatagramSocket();
            DatagramPacket answerForeingResolverPacket = new DatagramPacket(foreingResolver, foreingResolver.length, foreingResolverAddress, 53);
            socket.send(answerForeingResolverPacket);
        }
        return foreingResolver;
    };
    ...
    // Eliminar el último punto sobrante
        ...
        if(ipAddress != null){
            ...
            // ############ Cuarta fila ############
            int ttl = 3600; // Valor TTL en segundos 1 hora
